...
import { motion, type Variants, type TargetAndTransition, type HTMLMotionProps, type Transition } from "motion/react"
            ...
            >
                {tokens.map((token, i) => (
                    <React.Fragment key={i}>
                        <motion.span variants={itemVariants} style={{ display: "inline-block", whiteSpace: "normal" }}>
                            {token.trim()}
                        </motion.span>
                        {/\s$/.test(token) ? " " : null}
                    </React.Fragment>
